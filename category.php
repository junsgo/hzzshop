...
//    if ($brand > 0)
//    {
//        $sql = "SELECT brand_name FROM " .$GLOBALS['hhs']->table('brand'). " WHERE brand_id = '$brand'";
...
//        {
//            $sql = "SELECT a.attr_name FROM " . $hhs->table('attribute') . " AS a, " . $hhs->table('goods_attr') . " AS ga, " . $hhs->table('goods') . " AS g WHERE ($children OR " . get_extension_goods($children) . ") AND a.attr_id = ga.attr_id AND g.goods_id = ga.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND a.attr_id='$value'";
//            if($temp_name = $db->getOne($sql))
//            {
//                $all_attr_list[$key]['filter_attr_name'] = $temp_name;
//
//                $sql = "SELECT a.attr_id, MIN(a.goods_attr_id ) AS goods_id, a.attr_value AS attr_value FROM " . $hhs->table('goods_attr') . " AS a, " . $hhs->table('goods') .
//                       " AS g" .
...
//        {
//            $ext_sql = "SELECT DISTINCT(b.goods_id) FROM " . $hhs->table('goods_attr') . " AS a, " . $hhs->table('goods_attr') . " AS b " .  "WHERE ";
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name,g.goods_number,g.guige,  g.sales_num, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
            ...
            "WHERE $where $ext ORDER BY sort_order DESC";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$arr[$row['goods_id']]['guige']             = $row['guige'];
        ...
        $arr[$row['goods_id']]['team_discount']    = number_format($row['team_price']/$row['shop_price']*10,1);
		$arr[$row['goods_id']]['buy_num']       = ((buysum($row['goods_id'])+$row['sales_num']));
    }
	#print_r($arr);
    return $arr;
}
...
function get_cagtegory_goods_count($children, $brand = 0, $min = 0, $max = 0, $ext='')
{
    ...
    /* 返回商品总数 */
    return $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' . $GLOBALS['hhs']->table('goods') . " AS g WHERE $where $ext");
}
