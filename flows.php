 ...
 * ============================================================================
 * $Author: douqinghua $
 * $Id: flows.php 17218 2011-01-24 04:10:41Z douqinghua $
 */
...
elseif ($_REQUEST['step'] == 'login')
{
    ...
    else
    {
        ...
        if (!empty($_POST['act']) && $_POST['act'] == 'signin')
        {
            ...
            {
                $_SESSION['login_fail']++;
                show_message($_LANG['signin_failed'], '', 'flows.php?step=login');
            }
        }
...
elseif ($_REQUEST['step'] == 'address_list')
{
    ...
    if(empty($consignee_list)){
    	hhs_header("location:flows.php?step='edit_consignee';");
    	exit();
    }
   ...
   // $smarty->assign('s_address_id', get_user_address_id($_SESSION['user_id']));
    $smarty->assign('address_row',$address_row);
    /*
    $forward="flows.php?step=checkout";
...
elseif ($_REQUEST['step'] == 'checkout')
{
	...
	{
	    $forward="flows.php?step=checkout";
		hhs_header("Location: flows.php?step=edit_consignee&back_url=".urlencode($forward));
        exit;
	}else{
  ...
  /* 获得用户所有的收货人信息 */
    ...
    $smarty->assign('shipping_list',   $shipping_list);
    //$smarty->assign('shipping_list_count',   count($shipping_list));
    ...
    if(isset($payment_list))
    {
        foreach ($payment_list as $key => $payment)
        {
            ...
            if ($payment['pay_code'] == 'wxpay'){
                $smarty->assign('pay_id', $payment['pay_id']);
            }
        }
    }
...
elseif ($_REQUEST['step'] == 'done')
{
   ...
   /* 判断是不是实体商品 */
    ...
    /*记录同一个团的标志
    if($flow_type==5){
        $sql="select * from ".$hhs->table('order_info')."  where extension_id=".$_SESSION['extension_id']." and team_first=1 and team_status= 1 ";
        $current_team=$db->getRow($sql);
        if(!empty($current_team)) {
            //$order['team_first']=0;
            $order['team_sign']=$current_team['order_id'];
        } else{
            //$order['team_first']=1;
        }
    }*/
    ...
    /* 修改团购 
    if ($order['extension_code']=='team_goods' && $order['team_first']=1)
    {
        $sql = "UPDATE ". $hhs->table('order_info') ." SET team_sign=".$order['order_id']."  WHERE order_id=".$order['order_id'];
        $db->query($sql);
    } */
    /* 处理余额、积分、优惠劵 */
...
elseif ($_REQUEST['step'] == 'json_done')
{
    ...
    */
    ...
    $shipping_info=shipping_info($order['shipping_id']);
    $order['shipping_name']=$shipping_info['shipping_name'];
    $order['point_id'] = intval($_POST['point_id']);
    /* 扩展信息 */
    if (isset($_SESSION['flow_type']) && intval($_SESSION['flow_type']) != CART_GENERAL_GOODS)
    {
        $order['extension_code'] = $_SESSION['extension_code'];
