 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: lib_goods.php 17217 2014-05-12 06:29:08Z pangbin $
*/
...
function get_cate_index_tree($cat_id = 0)
{

        $sql = 'SELECT cat_id,cat_name,cat_desc,cat_img ,parent_id, show_in_nav ' .
                'FROM ' . $GLOBALS['hhs']->table('category') .
                "WHERE show_in_nav = 1 ORDER BY sort_order ASC, cat_id ASC";
...
function get_categories_tree($cat_id = 0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['hhs']->table('category') . " WHERE cat_id = '$cat_id'";
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
    */
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['hhs']->table('category') . " WHERE parent_id = '$parent_id' ";
	
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT cat_id,cat_name,cat_desc,cat_img ,parent_id,is_show ' .
                'FROM ' . $GLOBALS['hhs']->table('category') .
...
{
    $three_arr = array();
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['hhs']->table('category') . " WHERE parent_id = '$tree_id' ";
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if ($data === false)
        {
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order ' .
               ' FROM ' . $GLOBALS['hhs']->table('goods') . ' AS g ' .
               ' LEFT JOIN ' . $GLOBALS['hhs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
               ' WHERE g.is_mall=1 and g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND (g.is_best = 1 OR g.is_new =1 OR g.is_hot = 1) '.
        ...
        $sql = 'SELECT g.goods_id,g.goods_number, g.goods_name, g.guige, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
        ...
        foreach ($result AS $idx => $row)
        {
			...
			$goods[$idx]['little_img']= get_image_path($row['goods_id'], $row['little_img'], true);
...
function get_goods_info($goods_id)
{
    ...
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
            ...
            'LEFT JOIN ' . $GLOBALS['hhs']->table('comment') . ' AS r '.
                'ON r.id_value = g.goods_id AND comment_type = 0 AND r.parent_id = 0 AND r.status = 1 ' .
            'LEFT JOIN ' . $GLOBALS['hhs']->table('bonus_type') . ' AS m ' .
    ...
    if ($row !== false)
    {
        ...
        $row['team_price_formated'] = price_format($row['team_price'],false);
    ...
    if (!empty($attr))
    {
        foreach ($attr['lnk'] AS $key => $val)
        {
            ...
            $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                    ...
                    'FROM ' . $GLOBALS['hhs']->table('goods') . ' AS g ' .
                    'LEFT JOIN ' . $GLOBALS['hhs']->table('goods_attr') . ' as a ON g.goods_id = a.goods_id ' .
                    "LEFT JOIN " . $GLOBALS['hhs']->table('member_price') . " AS mp ".
                        "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    /* 分类信息 */
    $sql = 'SELECT cat_name FROM ' . $GLOBALS['hhs']->table('category') . " WHERE cat_id = '$cat_id'";
...
function assign_brand_goods($brand_id, $num = 0, $cat_id = 0,$order_rule = '')
{
    ...
    /* 分类信息 */
    $sql = 'SELECT brand_name FROM ' . $GLOBALS['hhs']->table('brand') . " WHERE brand_id = '$brand_id'";
...
function get_extension_goods($cats)
{
    ...
    $sql = 'SELECT goods_id FROM ' . $GLOBALS['hhs']->table('goods_cat') . " AS g WHERE $cats";
...
function spec_price($spec)
{
    if (!empty($spec))
    {
        ...
        $sql = 'SELECT SUM(attr_price) AS attr_price FROM ' . $GLOBALS['hhs']->table('goods_attr') . " WHERE $where";
...
{
    $sql = "SELECT * FROM " . $GLOBALS['hhs']->table('goods_activity') . " WHERE act_id = '$act_id'";
    ...
    /* 查询出价用户数和最后出价 */
    $sql = "SELECT COUNT(DISTINCT bid_user) FROM " . $GLOBALS['hhs']->table('auction_log') .
            " WHERE act_id = '$act_id'";
    $auction['bid_user_count'] = $GLOBALS['db']->getOne($sql);
    if ($auction['bid_user_count'] > 0)
    {
        $sql = "SELECT a.*, u.user_name " .
                "FROM " . $GLOBALS['hhs']->table('auction_log') . " AS a, " .
...
{
    $sql = "SELECT * FROM " . $GLOBALS['hhs']->table('favourable_activity') .
            " WHERE act_id = '$act_id'";
    $row = $GLOBALS['db']->getRow($sql);
    if (!empty($row))
    {
...
{
    $sql = "SELECT * FROM " . $GLOBALS['hhs']->table('wholesale') .
            " WHERE act_id = '$act_id'";
    $row = $GLOBALS['db']->getRow($sql);
    if (!empty($row))
    {
...
function get_goods_attr($goods_id)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name " .
            "FROM " . $GLOBALS['hhs']->table('goods') . " AS g, " . $GLOBALS['hhs']->table('attribute') . " AS a " .
            "WHERE g.goods_id = '$goods_id' " .
...
function get_goods_fittings($goods_list = array())
{
    ...
    $sql = 'SELECT gg.parent_id, ggg.goods_name AS parent_name, gg.goods_id, gg.goods_price, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price ".
            'FROM ' . $GLOBALS['hhs']->table('group_goods') . ' AS gg ' .
            'LEFT JOIN ' . $GLOBALS['hhs']->table('goods') . 'AS g ON g.goods_id = gg.goods_id ' .
            "LEFT JOIN " . $GLOBALS['hhs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = gg.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            "LEFT JOIN " . $GLOBALS['hhs']->table('goods') . " AS ggg ON ggg.goods_id = gg.parent_id ".
...
function get_products_info($goods_id, $spec_goods_attr_id)
{
    ...
    if(isset($goods_attr_array['sort']))
    {
        ...
        $sql = "SELECT * FROM " .$GLOBALS['hhs']->table('products'). " WHERE goods_id = '$goods_id' AND goods_attr = '$goods_attr' LIMIT 0, 1";
...
function get_categories_tree_pro($cat_id = 0)
{
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
    */
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['hhs']->table('category') . " WHERE parent_id = '$parent_id' AND is_show = 1 ";
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        /* 获取当前分类及其子分类 */
        ...
        foreach ($res AS $row)
        {
			$cat_id = $row['cat_id'];
			$children = get_children($cat_id);
			$cat = $GLOBALS['db']->getRow('SELECT cat_name, keywords, cat_desc, style, grade, filter_attr, parent_id FROM ' . $GLOBALS['hhs']->table('category') .
        " WHERE cat_id = '$cat_id'");

			/* 获取分类下文章 */
			$sql = 'SELECT a.article_id, a.title, ac.cat_name, a.add_time, a.file_url, a.open_type FROM '.$GLOBALS['hhs']->table('article_cat').' AS ac RIGHT JOIN '.$GLOBALS['hhs']->table('article')." AS a ON a.cat_id=ac.cat_id AND a.is_open = 1 WHERE ac.cat_name='$row[cat_name]' ORDER BY a.article_type,a.article_id DESC LIMIT 4 "	;
			...
			$sql = "SELECT b.brand_id, b.brand_name,  b.brand_logo, COUNT(*) AS goods_num, IF(b.brand_logo > '', '1', '0') AS tag ".
					"FROM " . $GLOBALS['hhs']->table('brand') . "AS b, ".
						$GLOBALS['hhs']->table('goods') . " AS g LEFT JOIN ". $GLOBALS['hhs']->table('goods_cat') . " AS gc ON g.goods_id = gc.goods_id " .
...
function buysum($goods_id)

{

    $sql = "select sum(goods_number) from " . $GLOBALS['hhs']->table('order_goods') . " AS g ,".$GLOBALS['hhs']->table('order_info') . " AS o WHERE o.order_id=g.order_id and g.goods_id = " . $goods_id . " and o.order_status=5 " ;
