 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: lib_insert.php 17217 2014-05-12 06:29:08Z pangbin $
*/
...
function insert_query_info()
{
    ...
    $online_count = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['hhs']->table('sessions'));
...
{
    $str = '';
    if (!empty($_COOKIE['HHS']['history']))
    {
        $where = db_create_in($_COOKIE['HHS']['history'], 'goods_id');
        $sql   = 'SELECT goods_id, goods_name, goods_thumb, shop_price FROM ' . $GLOBALS['hhs']->table('goods') .
...
function insert_ads($arr)
{
    ...
    if (!empty($arr['num']) && $arr['num'] != 1)
    {
        $sql  = 'SELECT a.ad_id, a.position_id, a.media_type, a.ad_link, a.ad_code, a.ad_name, p.ad_width, ' .
                ...
                'FROM ' . $GLOBALS['hhs']->table('ad') . ' AS a '.
                'LEFT JOIN ' . $GLOBALS['hhs']->table('ad_position') . ' AS p ON a.position_id = p.position_id ' .
...
function insert_member_info()
{
    ...
    {
        if (!empty($_COOKIE['HHS']['username']))
        {
            $GLOBALS['smarty']->assign('hhs_username', stripslashes($_COOKIE['HHS']['username']));
        }
...
function insert_bought_notes($arr)
{
    ...
    $sql = 'SELECT u.user_name, og.goods_number, oi.add_time, IF(oi.order_status IN (2, 3, 4), 0, 1) AS order_status ' .
           'FROM ' . $GLOBALS['hhs']->table('order_info') . ' AS oi LEFT JOIN ' . $GLOBALS['hhs']->table('users') . ' AS u ON oi.user_id = u.user_id, ' . $GLOBALS['hhs']->table('order_goods') . ' AS og ' .
...
function insert_cart_num()
{
    ...
    $output = $GLOBALS['smarty']->fetch('library/cart_num.lbi');
    return $output;
