 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: lib_main.php 17217 2014-05-12 06:29:08Z pangbin $
*/
...
function update_user_info()
{
    ...
    $sql = 'SELECT u.user_money,u.email, u.pay_points, u.user_rank, u.rank_points, '.
            ...
            ' FROM ' .$GLOBALS['hhs']->table('users'). ' AS u ' .
            ' LEFT JOIN ' .$GLOBALS['hhs']->table('user_bonus'). ' AS ub'.
            ' ON ub.user_id = u.user_id AND ub.used_time = 0 ' .
            ' LEFT JOIN ' .$GLOBALS['hhs']->table('bonus_type'). ' AS b'.
    ...
    if ($row = $GLOBALS['db']->getRow($sql))
    {
        ...
        if($row['user_rank'] >0)
        {
            $sql="SELECT special_rank from ".$GLOBALS['hhs']->table('user_rank')."where rank_id='$row[user_rank]'";
            if($GLOBALS['db']->getOne($sql)==='0' || $GLOBALS['db']->getOne($sql)===null)
            {   
                $sql="update ".$GLOBALS['hhs']->table('users')."set user_rank='0' where user_id='$_SESSION[user_id]'";
        ...
        if ($row['user_rank'] == 0)
        {
            ...
            $sql = 'SELECT rank_id, discount FROM ' . $GLOBALS['hhs']->table('user_rank') . " WHERE special_rank = '0' AND min_points <= " . intval($row['rank_points']) . ' AND max_points > ' . intval($row['rank_points']);
            if ($row = $GLOBALS['db']->getRow($sql))
            {
        ...
        {
            // 特殊等级
            $sql = 'SELECT rank_id, discount FROM ' . $GLOBALS['hhs']->table('user_rank') . " WHERE rank_id = '$row[user_rank]'";
            if ($row = $GLOBALS['db']->getRow($sql))
            {
                $_SESSION['user_rank'] = $row['rank_id'];
                $_SESSION['discount']  = $row['discount'] / 100.00;
    ...
    /* 更新登录时间，登录次数及登录ip */
    $sql = "UPDATE " .$GLOBALS['hhs']->table('users'). " SET".
...
function get_parent_cats($cat)
{
    ...
    $arr = $GLOBALS['db']->GetAll('SELECT cat_id, cat_name, parent_id FROM ' . $GLOBALS['hhs']->table('category'));
...
{
    $sql = 'SELECT id, number, type FROM ' . $GLOBALS['hhs']->table('template') .
...
{
    $sql = 'SELECT cat_name FROM ' . $GLOBALS['hhs']->table('article_cat') . " WHERE cat_id = '" . $id ."'";
 ...
 * @return  array
 */
function get_shop_about()
{
    $sql = 'SELECT cat_id, article_id, title, file_url, open_type ' .
            'FROM ' .$GLOBALS['hhs']->table('article').
            'WHERE cat_id = 1 AND is_open = 1 ' .
            'ORDER BY article_id';
    $abouts = $GLOBALS['db']->getAll($sql);

	foreach($abouts as $key=>$row)
    {
        $abouts[$key]['article_id']  = $row['article_id'];
        $abouts[$key]['title']       = $row['title'];
        $abouts[$key]['short_title'] = $GLOBALS['_CFG']['article_title_length'] > 0 ?
            sub_str($row['title'], $GLOBALS['_CFG']['article_title_length']) : $row['title'];
        $abouts[$key]['url']         = $row['open_type'] != 1 ?
            build_uri('about', array('aid' => $row['article_id']), $row['title']) : trim($row['file_url']);
    }
...
function get_shop_news()
{
    $sql = 'SELECT c.cat_id, c.cat_name, c.sort_order, a.article_id, a.title, a.file_url, a.open_type ' .
            ...
            'LEFT JOIN ' .$GLOBALS['hhs']->table('article_cat'). ' AS c ' .
            'ON a.cat_id = c.cat_id WHERE c.cat_type = 6 AND a.is_open = 1 ' .
    ...
    foreach ($res AS $key => $row)
    {
		...
		$arr[$row['cat_id']]['url']                       = build_uri('article_cat', array('acid'=> $row['cat_id']), $row['cat_name']);
        $arr[$row['cat_id']]['cat_name']                     = $row['cat_name'];
        $arr[$row['cat_id']]['article'][$key]['article_id']  = $row['article_id'];
		$arr[$row['cat_id']]['article'][$key]['cat_id']        = $row['cat_id'];
...
function get_vote($id = '')
{
    ...
    if ($vote_arr !== false && !empty($vote_arr))
    {
        ...
        $sql_option = 'SELECT v.*, o.option_id, o.vote_id, o.option_name, o.option_count ' .
                      'FROM ' . $GLOBALS['hhs']->table('vote') . ' AS v, ' .
                            $GLOBALS['hhs']->table('vote_option') . ' AS o ' .
        ...
        /* 总票数 */
        $sql = 'SELECT SUM(option_count) AS all_option FROM ' . $GLOBALS['hhs']->table('vote_option') .
...
function is_spider($record = true)
{
    ...
    foreach ($searchengine_bot AS $key => $value)
    {
        if (strpos($spider, $value) !== false)
        {
            ...
            if ($record === true)
            {
                $GLOBALS['db']->autoReplace($GLOBALS['hhs']->table('searchengine'), array('date' => local_date('Y-m-d'), 'searchengine' => $spider, 'count' => 1), array('count' => 1));
            }
...
function visit_stats()
{
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['hhs']->table('stats') . ' ( ' .
...
function save_searchengine_keyword($domain, $path)
{
    ...
    if (!empty($keywords))
    {
        ...
        {
            $keywords = hhs_iconv('GBK', 'UTF8', $keywords);
        }
        if (EC_CHARSET == 'gbk' && !in_array($searchengine, $gb_search))
        {
            $keywords = hhs_iconv('UTF8', 'GBK', $keywords);
        }

        $GLOBALS['db']->autoReplace($GLOBALS['hhs']->table('keywords'), array('date' => local_date('Y-m-d'), 'searchengine' => $searchengine, 'keyword' => addslashes($keywords), 'count' => 1), array('count' => 1));
    }
}
...
function recalculate_price()
{
    ...
    foreach ($res AS $row)
    {
        ...
        $goods_sql = "UPDATE " .$GLOBALS['hhs']->table('cart'). " SET goods_price = '$goods_price' ".
    ...
    /* 删除赠品，重新选择 */
    $GLOBALS['db']->query('DELETE FROM ' . $GLOBALS['hhs']->table('cart') .
...
function get_user_bonus($user_id = 0)
{
    ...
    $sql = "SELECT SUM(bt.type_money) AS bonus_value, COUNT(*) AS bonus_count ".
            "FROM " .$GLOBALS['hhs']->table('user_bonus'). " AS ub, ".
                $GLOBALS['hhs']->table('bonus_type') . " AS bt ".
...
function set_affiliate()
{
    ...
    if (!empty($_GET['u']) && $config['on'] == 1)
    {
        if(!empty($config['config']['expire']))
        {
            ...
            setcookie('hhshop_affiliate_uid', intval($_GET['u']), gmtime() + 3600 * $config['config']['expire'] * $c);
        }
        else
        {
            setcookie('hhshop_affiliate_uid', intval($_GET['u']), gmtime() + 3600 * 24); // 过期时间为 1 天
        }
    }
}
...
{
    if (!empty($_COOKIE['hhshop_affiliate_uid']))
    {
        $uid = intval($_COOKIE['hhshop_affiliate_uid']);
        if ($GLOBALS['db']->getOne('SELECT user_id FROM ' . $GLOBALS['hhs']->table('users') . "WHERE user_id = '$uid'"))
        {
            return $uid;
        }
        else
        {
            setcookie('hhshop_affiliate_uid', '', 1);
        }
    }
...
function article_categories_tree($cat_id = 0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['hhs']->table('article_cat') . " WHERE cat_id = '$cat_id'";
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
    */
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['hhs']->table('article_cat') . " WHERE parent_id = '$parent_id'";
    if ($GLOBALS['db']->getOne($sql))
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT a.cat_id, a.cat_name, a.sort_order AS parent_order, a.cat_id, ' .
                    'b.cat_id AS child_id, b.cat_name AS child_name, b.sort_order AS child_order ' .
                'FROM ' . $GLOBALS['hhs']->table('article_cat') . ' AS a ' .
                'LEFT JOIN ' . $GLOBALS['hhs']->table('article_cat') . ' AS b ON b.parent_id = a.cat_id ' .
...
function get_article_parent_cats($cat)
{
    ...
    $arr = $GLOBALS['db']->GetAll('SELECT cat_id, cat_name, parent_id FROM ' . $GLOBALS['hhs']->table('article_cat'));
...
function get_library_number($library, $template = null)
{
    ...
    if (!isset($lib_list[$template]))
    {
        ...
        $sql = "SELECT library, number FROM " . $GLOBALS['hhs']->table('template') .
...
{
    $sql = 'SELECT * FROM '. $GLOBALS['hhs']->table('nav') . '
