...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0,$rec_type=0,$team_sign=0)
{
    ...
    if ($num > 0)
    {
        ...
        else //购物车没有此物品，则插入
        {
            ...
            if($rec_type==5){
                $parent['goods_price']=$goods['team_price'];
            }
...
function user_bonus($user_id, $goods_amount = 0)
{
    ...
    $sql = "SELECT t.type_id, t.type_name, t.type_money, b.bonus_id,t.use_start_date, t.use_end_date  " .
...
function get_consignee($user_id)
{
    ...
    else
    {
        ...
        if ($user_id > 0)
        {
            /* 取默认地址 */
            $sql = "SELECT ua.*".
                    " FROM " . $GLOBALS['hhs']->table('user_address') . "AS ua, ".$GLOBALS['hhs']->table('users').' AS u '.
...
function order_query_sql($type = 'finished', $alias = '')
{
    ...
    elseif ($type == 'await_ship_team')
    {
        return " AND   {$alias}order_status " .
               ...
               " AND ( {$alias}pay_status " . db_create_in(array(PS_PAYED, PS_PAYING,PS_REFUNDED)) . " OR {$alias}pay_id " . db_create_in(payment_id_list(true)) . ") ";
    }
    /* 待发货订单 */
        ...
        foreach ($bonus_list AS $bonus)
        {
            ...
            if($bonus['is_share']==1){
                ...
                for($i=0;$i<$bonus['share_number'];$i++){
                    $sql = "INSERT INTO " . $GLOBALS['hhs']->table('user_bonus') . " (bonus_type_id, send_user_id,send_order_id,send_id) " .
                        "VALUES('$bonus[type_id]', '$user[user_id]','$order_id','$send_id')";
                   $GLOBALS['db']->query($sql);
                    /*
                ...
                $sql = "INSERT INTO " . $GLOBALS['hhs']->table('user_bonus') . " (bonus_type_id, user_id,send_order_id) " .
                    "VALUES('$bonus[type_id]', '$user[user_id]','$order_id')";
                for ($i = 0; $i < $bonus['number']; $i++)
                {
                if (!$GLOBALS['db']->query($sql))
                {
...
function order_bonus($order_id)
{
    ...
    //发放的普通优惠券
    $sql = "SELECT b.type_id, b.type_money,b.is_share, SUM(o.goods_number) AS number,b.number as share_number,use_start_date,use_end_date " .
            "FROM " . $GLOBALS['hhs']->table('order_goods') . " AS o, " .
                      $GLOBALS['hhs']->table('goods') . " AS g, " .
...
//分享的优惠券
    ...
    /* 查询按订单发的优惠劵 */
    $sql = "SELECT type_id, type_money,is_share ,IFNULL(FLOOR('$amount' / min_amount), 1) AS number , number as share_number,use_start_date,use_end_date " .
            " FROM " . $GLOBALS['hhs']->table('bonus_type') .
            "WHERE send_type = '" . SEND_BY_ORDER . "' " .
