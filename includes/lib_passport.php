 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: lib_passport.php 17217 2014-05-12 06:29:08Z pangbin $
*/
...
function register($username, $password, $email, $other = array())
{
    ...
    else
    {
        ...
        if (isset($affiliate['on']) && $affiliate['on'] == 1)
        {
            ...
            if ($up_uid)
            {
                if (!empty($affiliate['config']['level_register_all']))
                {
                    ...
                    {
                        $rank_points = $GLOBALS['db']->getOne("SELECT rank_points FROM " . $GLOBALS['hhs']->table('users') . " WHERE user_id = '$up_uid'");
                ...
                //设置推荐人
                $sql = 'UPDATE '. $GLOBALS['hhs']->table('users') . ' SET parent_id = ' . $up_uid . ' WHERE user_id = ' . $_SESSION['user_id'];

                $GLOBALS['db']->query($sql);
            }
        }
        //定义other合法的变量数组
        $other_key_array = array('msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone','openid','uname');
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['hhs']->table('users'), $update_data, 'UPDATE', 'user_id = ' . $_SESSION['user_id']);
...
function send_regiter_hash ($user_id)
{
    ...
    $validate_email = $GLOBALS['hhs']->url() . 'user.php?act=validate_email&hash=' . $hash;

    $sql = "SELECT user_name, email FROM " . $GLOBALS['hhs']->table('users') . " WHERE user_id = '$user_id'";
...
{
    $res = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['hhs']->table('admin_user') .
