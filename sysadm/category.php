...
if ($_REQUEST['act'] == 'edit')
{
    ...
    if ($cat_info['filter_attr'])
    {
        ...
        {
            $attr_cat_id = $db->getOne("SELECT cat_id FROM " . $hhs->table('attribute') . " WHERE attr_id = '" . intval($v) . "'");
    ...
    //分类是否存在首页推荐
    $res = $db->getAll("SELECT recommend_type FROM " . $hhs->table("cat_recommend") . " WHERE cat_id=" . $cat_id);
    if (!empty($res))
    {
        $cat_recommend = array();
        foreach($res as $data)
...
elseif($_REQUEST['act'] == 'add_category')
{
    ...
    {
        $sql = "INSERT INTO " . $hhs->table('category') . "(cat_name, parent_id, is_show)" .
...
if ($_REQUEST['act'] == 'update')
{
    ...
    $dat = $db->getRow("SELECT cat_name, show_in_nav FROM ". $hhs->table('category') . " WHERE cat_id = '$cat_id'");

    if ($db->autoExecute($hhs->table('category'), $cat, 'UPDATE', "cat_id='$cat_id'"))
    {
        if($cat['cat_name'] != $dat['cat_name'])
        {
            //如果分类名称发生了改变
            $sql = "UPDATE " . $hhs->table('nav') . " SET name = '" . $cat['cat_name'] . "' WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'";
            $db->query($sql);
        }
...
if ($_REQUEST['act'] == 'move_cat')
{
    ...
    /* 更新商品分类 */
    $sql = "UPDATE " .$hhs->table('goods'). " SET cat_id = '$target_cat_id' ".
           "WHERE cat_id = '$cat_id'";
    if ($db->query($sql))
    {
        /* 清除缓存 */
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    $cat_name = $db->getOne('SELECT cat_name FROM ' .$hhs->table('category'). " WHERE cat_id='$cat_id'");

    /* 当前分类下是否有子分类 */
    $cat_count = $db->getOne('SELECT COUNT(*) FROM ' .$hhs->table('category'). " WHERE parent_id='$cat_id'");

    /* 当前分类下是否存在商品 */
    $goods_count = $db->getOne('SELECT COUNT(*) FROM ' .$hhs->table('goods'). " WHERE cat_id='$cat_id'");

    /* 如果不存在下级子分类和商品，则删除之 */
    if ($cat_count == 0 && $goods_count == 0)
    {
        /* 删除分类 */
        $sql = 'DELETE FROM ' .$hhs->table('category'). " WHERE cat_id = '$cat_id'";
        if ($db->query($sql))
        {
            $db->query("DELETE FROM " . $hhs->table('nav') . "WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'");
            clear_cache_files();
...
//{
//    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['hhs']->table('category').
...
{
    $sql = "SELECT * FROM " .$GLOBALS['hhs']->table('category'). " WHERE cat_id='$cat_id' LIMIT 1";
...
function cat_update($cat_id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['hhs']->table('category'), $args, 'update', "cat_id='$cat_id'");
}
...
function get_attr_list()
{
    $sql = "SELECT a.attr_id, a.cat_id, a.attr_name ".
           ...
           $GLOBALS['hhs']->table('goods_type') . " AS c ".
...
function insert_cat_recommend($recommend_type, $cat_id)
{
    ...
    {
        //取得之前的分类
        $recommend_res = $GLOBALS['db']->getAll("SELECT recommend_type FROM " . $GLOBALS['hhs']->table("cat_recommend") . " WHERE cat_id=" . $cat_id);
        if (empty($recommend_res))
        {
            foreach($recommend_type as $data)
            {
                $data = intval($data);
                $GLOBALS['db']->query("INSERT INTO " . $GLOBALS['hhs']->table("cat_recommend") . "(cat_id, recommend_type) VALUES ('$cat_id', '$data')");
            }
        }
        else
        {
            ...
            {
                $GLOBALS['db']->query("DELETE FROM " . $GLOBALS['hhs']->table("cat_recommend") . " WHERE cat_id=$cat_id AND recommend_type " . db_create_in($delete_array));
            }
    ...
    {
        $GLOBALS['db']->query("DELETE FROM ". $GLOBALS['hhs']->table("cat_recommend") . " WHERE cat_id=" . $cat_id);
    }
}
