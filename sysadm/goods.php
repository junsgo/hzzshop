...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    if ($is_add)
    {
        ...
        if (!empty($_COOKIE['HHSCP']['last_choose']))
        {
            $last_choose = explode('|', $_COOKIE['HHSCP']['last_choose']);
        }
        $goods = array(
            'goods_id'      => 0,
            'goods_desc'    => '',
        ...
        $sql = "DELETE FROM " . $hhs->table('link_goods') .
        ...
        $sql = "DELETE FROM " . $hhs->table('group_goods') .
                " WHERE parent_id = 0 AND admin_id = '$_SESSION[admin_id]'";
        $db->query($sql);

        /* 关联文章 */
        $goods_article_list = array();
        $sql = "DELETE FROM " . $hhs->table('goods_article') .
                " WHERE goods_id = 0 AND admin_id = '$_SESSION[admin_id]'";
        $db->query($sql);

        /* 属性 */
        $sql = "DELETE FROM " . $hhs->table('goods_attr') . " WHERE goods_id = 0";
        $db->query($sql);
    ...
    {
        /* 商品信息 */
        $sql = "SELECT * FROM " . $hhs->table('goods') . " WHERE goods_id = '$_REQUEST[goods_id]'";
        $goods = $db->getRow($sql);
        ...
        if ($_REQUEST['act'] == 'copy')
        {
            ...
            $sql = "SELECT '0' AS goods_id, link_goods_id, is_double, '$_SESSION[admin_id]' AS admin_id" .
                    " FROM " . $hhs->table('link_goods') .
                    " WHERE goods_id = '$_REQUEST[goods_id]' ";
            $res = $db->query($sql);
            while ($row = $db->fetchRow($res))
            {
                $db->autoExecute($hhs->table('link_goods'), $row, 'INSERT');
            }
            ...
            $sql = "SELECT 0 AS parent_id, goods_id, goods_price, '$_SESSION[admin_id]' AS admin_id " .
                    "FROM " . $hhs->table('group_goods') .
                    " WHERE parent_id = '$_REQUEST[goods_id]' ";
            $res = $db->query($sql);
            while ($row = $db->fetchRow($res))
            {
                $db->autoExecute($hhs->table('group_goods'), $row, 'INSERT');
            }
            ...
            $sql = "SELECT 0 AS goods_id, article_id, '$_SESSION[admin_id]' AS admin_id " .
                    "FROM " . $hhs->table('goods_article') .
                    " WHERE goods_id = '$_REQUEST[goods_id]' ";
            $res = $db->query($sql);
            while ($row = $db->fetchRow($res))
            {
                $db->autoExecute($hhs->table('goods_article'), $row, 'INSERT');
            }
            ...
            $sql = "SELECT 0 AS goods_id, attr_id, attr_value, attr_price " .
                    "FROM " . $hhs->table('goods_attr') .
                    " WHERE goods_id = '$_REQUEST[goods_id]' ";
            $res = $db->query($sql);
            while ($row = $db->fetchRow($res))
            {
                $db->autoExecute($hhs->table('goods_attr'), addslashes_deep($row), 'INSERT');
            }
        }

        // 扩展分类
        $other_cat_list = array();
        $sql = "SELECT cat_id FROM " . $hhs->table('goods_cat') . " WHERE goods_id = '$_REQUEST[goods_id]'";
        ...
        /* 图片列表 */
        $sql = "SELECT * FROM " . $hhs->table('goods_gallery') . " WHERE goods_id = '$goods[goods_id]'";
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    {
        $max_id     = $is_insert ? $db->getOne("SELECT MAX(goods_id) + 1 FROM ".$hhs->table('goods')) : $_REQUEST['goods_id'];
    ...
    $team_num = !empty($_POST['team_num']) ? $_POST['team_num'] : 0;
    $team_price = !empty($_POST['team_price']) ? $_POST['team_price'] : 0;
    
    $little_img= $image->upload_image($_FILES['little_img']);
	
	$sales_num =  $_POST['sales_num'];  
	$is_mall = !empty($_POST['is_mall']) ? $_POST['is_mall'] : 0;
	$is_team = !empty($_POST['is_team']) ? $_POST['is_team'] : 0;
	$is_nearby = isset($_POST['is_nearby']) ? $_POST['is_nearby'] : 0;
	$guige =  !empty($_POST['guige']) ? $_POST['guige'] : '';
	
	$discount_type = isset($_POST['discount_type']) ? $_POST['discount_type'] : 0;
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $hhs->table('goods') . " (subscribe,limit_buy_one,limit_buy_bumber,discount_type,discount_amount,guige,is_nearby,is_mall,is_team,sales_num,little_img,team_num,team_price,goods_name, goods_name_style, goods_sn, " .
                ...
                "VALUES ('$subscribe','$limit_buy_one','$limit_buy_bumber','$discount_type','$discount_amount','$guige','$is_nearby','$is_mall','$is_team','$sales_num','$little_img','$team_num','$team_price','$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
    ...
    if ($goods_img !== false)
    {
        $db->query("UPDATE " . $hhs->table('goods') . " SET goods_img = '$goods_img' WHERE goods_id='$goods_id'");
    }

    if ($original_img !== false)
    {
        $db->query("UPDATE " . $hhs->table('goods') . " SET original_img = '$original_img' WHERE goods_id='$goods_id'");
    }

    if ($goods_thumb !== false)
    {
        $db->query("UPDATE " . $hhs->table('goods') . " SET goods_thumb = '$goods_thumb' WHERE goods_id='$goods_id'");
    }
    if ($little_img !== false)
    {
        $db->query("UPDATE " . $hhs->table('goods') . " SET little_img = '$little_img' WHERE goods_id='$goods_id'");
    }
    ...
    if (isset($img))
    {
        ...
        $sql = "INSERT INTO " . $hhs->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original) " .
    ...
    if (!$is_insert && isset($_POST['old_img_desc']))
    {
        ...
        {
            $sql = "UPDATE " . $hhs->table('goods_gallery') . " SET img_desc = '$img_desc' WHERE img_id = '$img_id' LIMIT 1";
            $db->query($sql);
        }
    }
    ...
    /* 记录上一次选择的分类和品牌 */
    setcookie('HHSCP[last_choose]', $catgory_id . '|' . $brand_id, gmtime() + 86400);
    /* 清空缓存 */
    clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit_goods_sn')
{
    ...
    $sql="SELECT goods_id FROM ". $hhs->table('products')."WHERE product_sn='$goods_sn'";
    if($db->getOne($sql))
    {
        make_json_error($_LANG['goods_sn_exists']);
    }
...
elseif ($_REQUEST['act'] == 'edit_team_num')
{
    ...
    if ($team_num < 0 || $team_num == 0  )
    {
        make_json_error('你设置了一个无效的值');
    }
    else
    {
        if ($exc->edit("team_num = '$team_num', last_update=" .gmtime(), $goods_id))
        {
            clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit_team_price')
{
    ...
    if ($team_price < 0 || $team_price == 0 )
    {
        make_json_error('你设置了一个无效的值');
    }
    else
    {
        if ($exc->edit("team_price = '$team_price', last_update=" .gmtime(), $goods_id))
        {
            clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit_sales_num')
{
    check_authz_json('goods_manage');
    ...
    {
        if ($exc->edit("sales_num = '$team_price', last_update=" .gmtime(), $goods_id))
        {
            clear_cache_files();
            make_json_result($team_price);
        }
    }
}
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'drop_goods')
{
    ...
    if ($goods['is_real'] != 1)
    {
        $sql = "DELETE FROM " . $hhs->table('virtual_card') . " WHERE goods_id = '$goods_id'";
...
elseif ($_REQUEST['act'] == 'add_link_goods')
{
    ...
    foreach ($linked_array AS $val)
    {
        ...
        {
            /* 双向关联 */
            $sql = "INSERT INTO " . $hhs->table('link_goods') . " (goods_id, link_goods_id, is_double, admin_id) " .
...
elseif ($_REQUEST['act'] == 'drop_link_goods')
{
    ...
    {
        $sql = "UPDATE " .$hhs->table('link_goods') . " SET is_double = 0 ".
                " WHERE link_goods_id = '$goods_id' AND goods_id " . $drop_goods_ids;
    }
    if ($goods_id == 0)
    {
...
elseif ($_REQUEST['act'] == 'add_group_goods')
{
    ...
    {
        $sql = "INSERT INTO " . $hhs->table('group_goods') . " (parent_id, goods_id, goods_price, admin_id) " .
...
elseif ($_REQUEST['act'] == 'get_article_list')
{
    ...
    $sql        = 'SELECT article_id, title FROM ' .$hhs->table('article'). $where.
...
elseif ($_REQUEST['act'] == 'add_goods_article')
{
    ...
    {
        $sql = "INSERT INTO " . $hhs->table('goods_article') . " (goods_id, article_id, admin_id) " .
...
elseif ($_REQUEST['act'] == 'drop_goods_article')
{
    ...
    $sql = "DELETE FROM " .$hhs->table('goods_article') . " WHERE " . db_create_in($articles, "article_id") . " AND goods_id = '$goods_id'";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit_product_sn')
{
    ...
    /* 修改 */
    $sql = "UPDATE " . $hhs->table('products') . " SET product_sn = '$product_sn' WHERE product_id = '$product_id'";
    $result = $db->query($sql);
    if ($result)
    {
        clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit_product_number')
{
    ...
    /* 修改货品库存 */
    $sql = "UPDATE " . $hhs->table('products') . " SET product_number = '$product_number' WHERE product_id = '$product_id'";
    $result = $db->query($sql);
    if ($result)
    {
        /* 修改商品库存 */
...
elseif ($_REQUEST['act'] == 'batch_product')
{
    ...
    if ($_POST['type'] == 'drop')
    {
        ...
        $sum = 0;
        $goods_id = 0;
        $sql = "SELECT product_id, goods_id, product_number FROM  " . $GLOBALS['hhs']->table('products') . " WHERE product_id $product_bound";
        ...
        if (!empty($product_array))
        {
            ...
            /* 删除货品 */
            $sql = "DELETE FROM " . $hhs->table('products') . " WHERE product_id $product_bound";
            if ($db->query($sql))
            {
                //记录日志
...
{
    $sql = "DELETE FROM " . $GLOBALS['hhs']->table('volume_price') .
    ...
    foreach ($price_list AS $key => $price)
    {
        ...
        if (!empty($price))
        {
            $sql = "INSERT INTO " . $GLOBALS['hhs']->table('volume_price') .
