...
/* 如果有新版本，升级 */
if (!isset($_CFG['hhs_version']))
{
    $_CFG['hhs_version'] = 'v2.0.5';
}

if (preg_replace('/(?:\.|\s+)[a-z]*$/i', '', $_CFG['hhs_version']) != preg_replace('/(?:\.|\s+)[a-z]*$/i', '', VERSION)
        && file_exists('../upgrade/index.php'))
{
    // 转到升级文件
    hhs_header("Location: ../upgrade/index.php\n");
    ...
    $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order')
{
    ...
    if (!empty($_COOKIE['HHSCP']['admin_id']) && !empty($_COOKIE['HHSCP']['admin_pass']))
    {
        ...
        $sql = 'SELECT user_id, user_name, password, action_list, last_login ' .
                ...
                " WHERE user_id = '" . intval($_COOKIE['HHSCP']['admin_id']) . "'";
        $row = $db->GetRow($sql);

        if (!$row)
        {
            // 没有找到这个记录
            setcookie($_COOKIE['HHSCP']['admin_id'],   '', 1);
            setcookie($_COOKIE['HHSCP']['admin_pass'], '', 1);
        ...
        {
            // 检查密码是否正确
            if (md5($row['password'] . $_CFG['hash_code']) == $_COOKIE['HHSCP']['admin_pass'])
            {
                !isset($row['last_time']) && $row['last_time'] = '';
                set_admin_session($row['user_id'], $row['user_name'], $row['action_list'], $row['last_time']);

                // 更新最后登录时间和IP
                $db->query('UPDATE ' . $hhs->table('admin_user') .
