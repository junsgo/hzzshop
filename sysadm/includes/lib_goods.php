...
{
    $sql = "SELECT * FROM " . $GLOBALS['hhs']->table('user_rank') .
           " ORDER BY min_points";
...
function get_member_price_list($goods_id)
{
    ...
    $sql = "SELECT user_rank, user_price FROM " .
           $GLOBALS['hhs']->table('member_price') .
...
function handle_member_price($goods_id, $rank_list, $price_list)
{
    ...
    foreach ($rank_list AS $key => $rank)
    {
        ...
        // 插入或更新记录
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['hhs']->table('member_price') .
        ...
        if ($GLOBALS['db']->getOne($sql) > 0)
        {
            ...
            if ($price < 0)
            {
                $sql = "DELETE FROM " . $GLOBALS['hhs']->table('member_price') .
                       " WHERE goods_id = '$goods_id' AND user_rank = '$rank' LIMIT 1";
            }
            else
            {
                $sql = "UPDATE " . $GLOBALS['hhs']->table('member_price') .
        ...
        else
        {
            ...
            {
                $sql = "INSERT INTO " . $GLOBALS['hhs']->table('member_price') . " (goods_id, user_rank, user_price) " .
...
{
    /* 查询现有的扩展分类 */
    $sql = "SELECT cat_id FROM " . $GLOBALS['hhs']->table('goods_cat') .
            " WHERE goods_id = '$goods_id'";
    ...
    if ($delete_list)
    {
        $sql = "DELETE FROM " . $GLOBALS['hhs']->table('goods_cat') .
                " WHERE goods_id = '$goods_id' " .
    ...
    {
        // 插入记录
        $sql = "INSERT INTO " . $GLOBALS['hhs']->table('goods_cat') .
                " (goods_id, cat_id) " .
...
{
    $sql = "UPDATE " . $GLOBALS['hhs']->table('link_goods') . " SET " .
            " goods_id = '$goods_id' " .
            " WHERE goods_id = '0'" .
...
{
    $sql = "UPDATE " . $GLOBALS['hhs']->table('group_goods') . " SET " .
            " parent_id = '$goods_id' " .
            " WHERE parent_id = '0'" .
            " AND admin_id = '$_SESSION[admin_id]'";
    $GLOBALS['db']->query($sql);
...
{
    $sql = "UPDATE " . $GLOBALS['hhs']->table('goods_article') . " SET " .
            " goods_id = '$goods_id' " .
            " WHERE goods_id = '0'" .
            " AND admin_id = '$_SESSION[admin_id]'";
    $GLOBALS['db']->query($sql);
...
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls)
{
    ...
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        if ($flag)
        {
            ...
            $sql = "INSERT INTO " . $GLOBALS['hhs']->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original) " .
                    "VALUES ('$goods_id', '$img_url', '$img_desc', '$thumb_url', '$img_original')";
            $GLOBALS['db']->query($sql);
            /* 不保留商品原图的时候删除原图 */
            if ($proc_thumb && !$GLOBALS['_CFG']['retain_original_img'] && !empty($img_original))
            {
                $GLOBALS['db']->query("UPDATE " . $GLOBALS['hhs']->table('goods_gallery') . " SET img_original='' WHERE `goods_id`='{$goods_id}'");
                @unlink('../' . $img_original);
            }
        }
...
function delete_goods($goods_id)
{
    ...
    /* 取得有效商品id */
    $sql = "SELECT DISTINCT goods_id FROM " . $GLOBALS['hhs']->table('goods') .
    ...
    /* 删除商品 */
    $sql = "DELETE FROM " . $GLOBALS['hhs']->table('goods') .
            " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);

    /* 删除商品的货品记录 */
    $sql = "DELETE FROM " . $GLOBALS['hhs']->table('products') .
            " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    ...
    /* 删除商品相册 */
    $sql = "DELETE FROM " . $GLOBALS['hhs']->table('goods_gallery') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);

    /* 删除相关表记录 */
    $sql = "DELETE FROM " . $GLOBALS['hhs']->table('collect_goods') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['hhs']->table('goods_article') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['hhs']->table('goods_attr') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['hhs']->table('goods_cat') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['hhs']->table('member_price') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['hhs']->table('group_goods') . " WHERE parent_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['hhs']->table('group_goods') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['hhs']->table('link_goods') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['hhs']->table('link_goods') . " WHERE link_goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['hhs']->table('tag') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['hhs']->table('comment') . " WHERE comment_type = 0 AND id_value " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);

    /* 删除相应虚拟商品记录 */
    $sql = "DELETE FROM " . $GLOBALS['hhs']->table('virtual_card') . " WHERE goods_id " . db_create_in($goods_id);
...
function get_linked_goods($goods_id)
{
    $sql = "SELECT lg.link_goods_id AS goods_id, g.goods_name, lg.is_double " .
            "FROM " . $GLOBALS['hhs']->table('link_goods') . " AS lg, " .
...
function get_goods_articles($goods_id)
{
    $sql = "SELECT g.article_id, a.title " .
            "FROM " .$GLOBALS['hhs']->table('goods_article') . " AS g, " .
                $GLOBALS['hhs']->table('article') . " AS a " .
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['hhs']->table('goods'). " AS g WHERE is_delete='$is_delete' $where";
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT team_num,team_price,limit_buy_one,goods_id,sales_num, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".
                    " FROM " . $GLOBALS['hhs']->table('goods') . " AS g WHERE is_delete='$is_delete' $where" .
...
function check_goods_specifications_exist($goods_id)
{
    ...
    $sql = "SELECT COUNT(a.attr_id)
            FROM " .$GLOBALS['hhs']->table('attribute'). " AS a, " .$GLOBALS['hhs']->table('goods'). " AS g
