 ...
 * ==========================================================
 * $Author: pangbin $
 * $Id: navigator.php 17217 2014-05-12 06:29:08Z pangbin $
 */
...
$exc = new exchange($hhs->table("nav"), $db, 'id', 'name');
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    elseif ($_REQUEST['step'] == 2)
    {
        ...
        $item_icon = $_REQUEST['item_icon'];
        
        $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $hhs->table('nav') . " WHERE type = '". $item_type ."'");
        ...
        if($item_ifshow == 1 && $item_type == 'middle')
        {
            ...
            if($arr)
            {
                ...
                $sql = "INSERT INTO " . $GLOBALS['hhs']->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type,item_icon) VALUES('$item_name','".$arr['type']."','".$arr['id']."','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type','$item_icon')";
            }
        }

        if(empty($sql))
        {
            $sql = "INSERT INTO " . $GLOBALS['hhs']->table('nav') . " (name,ifshow,vieworder,opennew,url,type,item_icon) VALUES('$item_name','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type','$item_icon')";
        }
        $db->query($sql);
        clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    if (empty($_REQUEST['step']))
    {
        ...
        $row = $db->getRow("SELECT * FROM " . $GLOBALS['hhs']->table('nav') . " WHERE id='$id'");
        $rt['item_name'] = $row['name'];
        $rt['item_url'] = $row['url'];
        $rt['item_vieworder'] = $row['vieworder'];
        $rt['item_icon'] = $row['item_icon'];
    ...
    elseif ($_REQUEST['step'] == 2)
    {
        ...
        $item_icon = $_REQUEST['item_icon'];
        $row = $db->getRow("SELECT ctype,cid,ifshow,type FROM " . $GLOBALS['hhs']->table('nav') . " WHERE id = '$id'");
        ...
        if($arr)
        {
            ...
            $sql = "UPDATE " . $GLOBALS['hhs']->table('nav') .
                " SET item_icon='$item_icon' ,name='$item_name',ctype='" . $arr['type'] . "',cid='" . $arr['id'] . "',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',type='$item_type' WHERE id='$id'";
        }
        else
        {
            //目标不是分类
            ...
            $sql = "UPDATE " . $GLOBALS['hhs']->table('nav') .
                " SET name='$item_name',ctype='',cid='',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',type='$item_type',item_icon='$item_icon' WHERE id='$id'";
        }
...
if ($_REQUEST['act'] == 'toggle_ifshow')
{
    ...
    $row = $db->getRow("SELECT type,ctype,cid FROM " . $GLOBALS['hhs']->table('nav') . " WHERE id = '$id' LIMIT 1");
...
function get_nav()
{
    ...
    if($result === false)
    {
        ...
        $sql = "SELECT count(*) FROM " .$GLOBALS['hhs']->table('nav');
...
*/
    ...
    $sysmain = array(
        array('首页','index.php'),
        array("我的订单",'user.php?act=order_list'),
        array('个人中心','user.php')
    );
    if($_CFG['open_team']){
        $sysmain[]=array('拼好货','group.php');
        $sysmain[]=array('我的团购','user.php?act=team_list');
    }
    $sysmain[] = array('-','-');
...
function nav_update($id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['hhs']->table('nav'), $args, 'update', "id='$id'");
}
...
function is_show_in_nav($type, $id)
{
    if($type == 'c')
    {
        $tablename = $GLOBALS['hhs']->table('category');
    }
    else
    {
        $tablename = $GLOBALS['hhs']->table('article_cat');
    }
