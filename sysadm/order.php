...
elseif ($_REQUEST['act'] == 'team_refund'){
    ...
    $sql="select * from ".$hhs->table('order_info')." where team_sign=".$_REQUEST['team_sign']." and team_status>0 ";
...
*/
	...
	if(!empty($order_list['orders'])){
		foreach($order_list['orders'] as $k=>$v){
		    $sql="select * from ".$hhs->table('order_goods')." as og left join ".
        ...
        if(is_file($newpath)) {
            ...
            if(!empty($list)){
                foreach($list as $t) {
                    ...
                    //$t = explode(',', $v);
                    $sql="select region_id from ".$hhs->table('region')." where region_name ='$t[9]' ";
                    $province=$db->getOne($sql);
                    $sql="select region_id from ".$hhs->table('region')." where region_name ='$t[10]' ";
                    $city=$db->getOne($sql);
                    $sql="select region_id from ".$hhs->table('region')." where region_name ='$t[11]' ";
                    $district=$db->getOne($sql);
                    //order_sn='$t[3]',
                    if(!empty($province)&&!empty($city)&&!empty($district)){
                        $field=" set province='$province',city='$city',district='$district',
                        address='$t[12]',mobile='$t[13]',consignee='$t[14]',
                        shipping_id='$t[16]',shipping_name='$t[17]',invoice_no='$t[18]' ";
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    if(!empty($order['point_id'])){
        $sql = "SELECT a.*,rp.region_name as province,rc.region_name as city,rd.region_name as district " .
            ...
            $GLOBALS['hhs']->table('region') ." as rd on a.district=rd.region_id where id=".$order['point_id'];
    	//$sql="select * from ".$hhs->table('shipping_point')." where id=".$order['point_id'];
    	$point_info=$db->getRow($sql);
 ...
 * @return void
 */
function order_list($refund_ex="",$is_page=true)
{
    ...
    if ($result === false)
    {
        ...
        $filter['type'] = empty($_REQUEST['type']) ? -1 : $_REQUEST['type'];
        ...
        $where = " WHERE 1 ";//and extension_code!='team_goods'
		if(!empty($refund_ex) )
		{
        ...
        {
            if($filter['type']==1){
                $where .= " AND o.extension_code='team_goods' ";
            }else{
                $where .= " AND o.extension_code!='team_goods' ";
            } 
        }
        ...
        switch($filter['composite_status'])
        {
            ...
            case CS_AWAIT_SHIP :
                ...
                //$where .= order_query_sql('await_ship');
                $where.= " and ((o.extension_code='team_goods' and o.team_status=2  ".order_query_sql('await_ship_team').") or (o.extension_code!='team_goods' ".order_query_sql('await_ship').") )";
                break; 
        ...
        /* 查询 */
        if($is_page){
            $sql = "SELECT o.transaction_id,o.team_status,o.team_sign,o.team_first, o.order_id, o.order_sn, o.add_time,o.pay_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
 ...
 */
        ...
        /* 查询 */
        $sql ="SELECT o.discount_type,o.discount_amount,u.uname,o.team_num,o.transaction_id,o.teammen_num,(o.team_num-o.teammen_num) as team_lack_num, o.team_status,o.team_sign,o.team_first,o.extension_code, o.order_id, o.order_sn, o.add_time,o.pay_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
            "o.pay_status, o.consignee, o.address, o.email, o.tel,  o.extension_id, " .
            "(" . order_amount_field('o.') . ") AS total_fee, " .
...
function check_utf($url){
    //这是思路1
    $temp1=iconv("UTF-8","GBK",$url);
    $temp2=iconv("GBK","UTF-8",$temp1);
