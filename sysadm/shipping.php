 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: shipping.php 17217 2014-05-12 06:29:08Z pangbin $
*/

define('IN_HHS', true);

require(dirname(__FILE__) . '/includes/init.php');
$exc = new exchange($hhs->table('shipping'), $db, 'shipping_code', 'shipping_name');
...
if ($_REQUEST['act'] == 'list')
{
    ...
    for ($i = 0; $i < count($modules); $i++)
    {
        ...
        /* 检查该插件是否已经安装 */
        $sql = "SELECT shipping_code, shipping_id, shipping_name, shipping_desc, insure, support_cod,shipping_order FROM " .$hhs->table('shipping'). " WHERE shipping_code='" .$modules[$i]['code']. "' ORDER BY shipping_order";
        $row = $db->GetRow($sql);
    ...
    /* 安装的配送方式 */
    $sql = "SELECT * FROM " .$hhs->table('shipping'). " WHERE enabled=1 ORDER BY shipping_order";
    $enabled_shipping = $db->getAll($sql);
    $smarty->assign('enabled_shipping', $enabled_shipping);
    $smarty->assign('default_shipping_id', $_CFG['default_shipping_id']);
...
elseif ($_REQUEST['act'] == 'default_shipping')
{
    
    $sql="select count(*)  from ".$hhs->table('shop_config')." where code='default_shipping_id' ";
    $default_shipping_id=$db->getOne($sql);
    if($default_shipping_id==0){	 	 	 	 	 	
        $sql="insert into ".$hhs->table('shop_config')."(parent_id,code,type,value,sort_order) values (6,'default_shipping_id','hidden','".$_REQUEST['default_shipping_id']."',1)  ";
        $db->query($sql);
    }else{
        $sql="update ".$hhs->table('shop_config')." set  value='".$_REQUEST['default_shipping_id']."' where  code='default_shipping_id' ";
        $db->query($sql);
    } 
    clear_cache_files();
    hhs_header("location:shipping.php?act=list");
}
/*------------------------------------------------------ */
//-- 安装配送方式
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'install')
{
    ...
    /* 检查该配送方式是否已经安装 */
    $sql = "SELECT shipping_id FROM " .$hhs->table('shipping'). " WHERE shipping_code = '$_GET[code]'";
    $id = $db->GetOne($sql);

    if ($id > 0)
    {
        /* 该配送方式已经安装过, 将该配送方式的状态设置为 enable */
        $db->query("UPDATE " .$hhs->table('shipping'). " SET enabled = 1 WHERE shipping_code = '$_GET[code]' LIMIT 1");
    }
    else
    {
        /* 该配送方式没有安装过, 将该配送方式的信息添加到数据库 */
        $insure = empty($modules[0]['insure']) ? 0 : $modules[0]['insure'];
        $sql = "INSERT INTO " . $hhs->table('shipping') . " (" .
...
elseif ($_REQUEST['act'] == 'uninstall')
{
    ...
    /* 获得该配送方式的ID */
    $row = $db->GetRow("SELECT shipping_id, shipping_name, print_bg FROM " .$hhs->table('shipping'). " WHERE shipping_code='$_GET[code]'");
    ...
    if ($row)
    {
        $all = $db->getCol("SELECT shipping_area_id FROM " .$hhs->table('shipping_area'). " WHERE shipping_id='$shipping_id'");
        $in  = db_create_in(join(',', $all));

        $db->query("DELETE FROM " .$hhs->table('area_region'). " WHERE shipping_area_id $in");
        $db->query("DELETE FROM " .$hhs->table('shipping_area'). " WHERE shipping_id='$shipping_id'");
        $db->query("DELETE FROM " .$hhs->table('shipping'). " WHERE shipping_id='$shipping_id'");
