...
$exc = new exchange($hhs->table('shipping_area'), $db, 'shipping_area_id', 'shipping_area_name');
...
elseif ($_REQUEST['act'] == 'add' && !empty($_REQUEST['shipping']))
{
    ...
    $shipping = $db->getRow("SELECT shipping_name, shipping_code FROM " .$hhs->table('shipping'). " WHERE shipping_id='$_REQUEST[shipping]'");
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    {
        $shipping_code = $db->getOne("SELECT shipping_code FROM " .$hhs->table('shipping').
...
function get_shipping_area_list($shipping_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $sql = "SELECT r.region_name " .
                "FROM " . $GLOBALS['hhs']->table('area_region'). " AS a, " .
                    $GLOBALS['hhs']->table('region') . " AS r ".
