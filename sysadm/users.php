...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    /* 取出注册扩展字段 */
    $sql = 'SELECT * FROM ' . $hhs->table('reg_fields') . ' WHERE type < 2 AND display = 1 AND id != 6 ORDER BY dis_order, id';
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT u.user_name, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.msn, u.office_phone, u.home_phone, u.mobile_phone".
        " FROM " .$hhs->table('users'). " u LEFT JOIN " . $hhs->table('users') . " u2 ON u.parent_id = u2.user_id WHERE u.user_id='$_GET[id]'";
...
elseif ($_REQUEST['act'] == 'address_list')
{
    ...
    $sql = "SELECT a.*, c.region_name AS country_name, p.region_name AS province, ct.region_name AS city_name, d.region_name AS district_name ".
           " FROM " .$hhs->table('user_address'). " as a ".
           " LEFT JOIN " . $hhs->table('region') . " AS c ON c.region_id = a.country " .
           " LEFT JOIN " . $hhs->table('region') . " AS p ON p.region_id = a.province " .
           " LEFT JOIN " . $hhs->table('region') . " AS ct ON ct.region_id = a.city " .
           " LEFT JOIN " . $hhs->table('region') . " AS d ON d.region_id = a.district " .
           " WHERE user_id='$id'";
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($filter['rank'])
        {
            $sql = "SELECT min_points,is_subscribe, max_points, special_rank FROM ".$GLOBALS['hhs']->table('user_rank')." WHERE rank_id = '$filter[rank]'";
