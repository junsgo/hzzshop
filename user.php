...
/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'weixin_login', 'profile', 'order_list', 'team_list','order_detail','team_detail','edit_consignee', 'address_list', 'edit_address', 'collection_list',
'message_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer', 'refund', 'act_refund','setting');
...
elseif ($action == 'profile')
{
    ...
    $sql = 'SELECT reg_field_id, content ' .
           'FROM ' . $hhs->table('reg_extend_info') .
...
elseif ($action == 'act_edit_profile')
{
    ...
    /* 更新用户扩展字段的数据 */
    $sql = 'SELECT id FROM ' . $hhs->table('reg_fields') . ' WHERE type = 0 AND display = 1 ORDER BY dis_order, id';   //读出所有扩展字段的id
    $fields_arr = $db->getAll($sql);
    ...
    foreach ($fields_arr AS $val)       //循环更新扩展用户信息
    {
        ...
        if(isset($_POST[$extend_field_index]))
        {
            ...
            $sql = 'SELECT * FROM ' . $hhs->table('reg_extend_info') . "  WHERE reg_field_id = '$val[id]' AND user_id = '$user_id'";
            if ($db->getOne($sql))      //如果之前没有记录，则插入
            {
                $sql = 'UPDATE ' . $hhs->table('reg_extend_info') . " SET content = '$temp_field_content' WHERE reg_field_id = '$val[id]' AND user_id = '$user_id'";
            }
            else
            {
                $sql = 'INSERT INTO '. $hhs->table('reg_extend_info') . " (`user_id`, `reg_field_id`, `content`) VALUES ('$user_id', '$val[id]', '$temp_field_content')";
            }
            $db->query($sql);
        }
    }
...
elseif ($action == 'order_list')
{
    ...
    {
        $where = " and order_status in (0,1,5)  and shipping_status=1 ";
    }
...
elseif ($action == 'team_list')
{
    ...
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$hhs->table('order_info'). " WHERE user_id = '$user_id' and extension_code='team_goods' and team_status>0 ");

    $pager  = get_pager('user.php', array('act' => $action), $record_count, $page);

    $orders = get_user_team_orders($user_id, $pager['size'], $pager['start']);
    $merge  = get_user_merge($user_id);
...
elseif ($action == 'account_deposit')
{
    ...
    $sql="select pay_id from ".$hhs->table('payment')." where pay_code ='wxpay' ";
...
else if ($action == 'track_packages')
{
    ...
    $sql = "SELECT order_id,order_sn,invoice_no,shipping_id FROM " .$hhs->table('order_info').
...
elseif ($action == 'transform_points')
{
    ...
    $sql = "SELECT user_id, user_name, pay_points, rank_points FROM " . $hhs->table('users')  . " WHERE user_id='$user_id'";
    $row = $db->getRow($sql);
...
elseif ($action == 'act_transform_points')
{
    ...
    /* 取出用户数据 */
    $sql = "SELECT user_name, user_id, pay_points, rank_points FROM " . $hhs->table('users') . " WHERE user_id='$user_id'";
    $row = $db->getRow($sql);
...
elseif ($action == 'act_transform_ucenter_points')
{
    ...
    if ($result === true)
    {
        $sql = "UPDATE " . $hhs->table('users') . " SET {$shop_points[$fromcredits]}={$shop_points[$fromcredits]}-'$exchange_amount' WHERE user_id='{$row['user_id']}'";
        $db->query($sql);
        $sql = "INSERT INTO " . $hhs->table('account_log') . "(user_id, {$shop_points[$fromcredits]}, change_time, change_desc, change_type)" . " VALUES ('{$row['user_id']}', '-$exchange_amount', '". gmtime() ."', '" . $cfg['uc_lang']['exchange'] . "', '98')";
        $db->query($sql);
