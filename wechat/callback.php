        ...
        if (!empty($postStr) or $debug == 1){
            ...
            if ($postObj->MsgType == 'event') {
                ...
                if ($postObj->Event == 'subscribe'){
					...
					$gzshb = $this->coupon($db, $fromUsername,$base_url);
...
//				exit;
    ...
    protected function coupon($db, $fromUsername,$base_url) {
        ...
        if (!empty($retc['coupon'])&&$ret['type_id']>0) {
			...
			if($ret['type_id']>0)
			{
				  ...
				  if (($time >= $ret['send_start_date']) or ($time <= $ret['send_end_date'])) {
					...
					//  if (!empty($ret['bonus_sn'])) {
						...
						$new_bonus_sn = $db->getOne("SELECT `bonus_sn` FROM `$thistable` where is_attention_send='0'  ORDER BY RAND() LIMIT 1");
					  if(!empty($new_bonus_sn))
					  {
				   
                   // $coupon = $user_bonus[$bonus_rand];
                       $contentStr = "\r\n关注送". $type_money . "元红包:" . $new_bonus_sn . "过期时间:$use_end_date"."<a href='".$base_url."/user.php?act=bonus&send_bouns=$new_bonus_sn'>立即绑定</a>";
