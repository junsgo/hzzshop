 ...
 * 所有接口的基类
 */
class Common_util_pub extends WxPayConf_pub
{
	...
	public function getSign($Obj)
	{
		...
		//签名步骤二：在string后加入KEY        if($this->wxpay_key){            $String = $String."&key=".$this->wxpay_key;        }
...
public function filepost($url, $post = null) {       
	...
	*/
		...
		//$data= $this->filepost($url,$xml);
		//返回结果
		//$this->printErr('ceshi',$data);
		if($data)
		{
			//curl_close($ch);
			return $data;
	...
	function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT, SSLCERT_PATH);//WxPayConf_pub::
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, SSLKEY_PATH);//WxPayConf_pub::
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
...
class NativeCall_pub extends Wxpay_server_pub
{
	...
	function createXml()
	{
		if($this->returnParameters["return_code"] == "SUCCESS"){
		   	$this->returnParameters["appid"] = $this->wxpay_app_id;//公众账号ID
		   	$this->returnParameters["mch_id"] = $this->wxpay_mchid;//商户号
...
class JsApi_pub extends Common_util_pub
{
	...
	{
		$urlObj["appid"] = $this->wxpay_app_id;
	...
	{
		$urlObj["appid"] = $this->wxpay_app_id;
		$urlObj["secret"] = $this->wxpay_app_secret;
	...
	{
		$jsApiObj["appId"] = $this->wxpay_app_id;
		$timeStamp = time();
	...
		public function getbutton($code,$returnrul,$order=null){		    if($order['extension_code']=='team_goods'&&$order['team_first']==2){	    	$is_check=1;	    }else{	        $is_check=0;	    }	    if(empty($order[team_sign])){	        $team_sign=0;	    }else{	        $team_sign=$order[team_sign];	    }	    $button = <<<EOT	<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">	<html xmlns="http://www.w3.org/1999/xhtml">	<head>	<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />	<title>微信支付</title>	</head>	<body><a class="state_btn_2" href='javascript:callpay({$code},"{$returnrul}",{$is_check},{$team_sign});' >去支付</a></body>	</html>	EOT;		//<div style="text-align:center"><input type="button" onclick="callpay()" value="立即使用微信支付" /></div>        return $button;		}	
	public function getbutton2($code,$returnrul,$order=null){
		$button = <<<EOT
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
...
<script type="text/javascript">
	...
	</script><a class="state_btn_2" href="javascript:callpay();"   >去支付</a>
